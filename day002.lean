...
-- 学「おじいちゃん, 今更だけど, `a b c : ℝ` と, `a b c ∈ ℝ` は何が違うの?」
-- ワシ「意味は同じ意味だ. これに関わる話で, 昨日言った『依存型理論』の話を少しする.」
...
-- 学「よくわからないけど, 型は何種類あるの?」
-- ワシ「無限だよ. 項に応じて型を作れる. そのため, 表現の幅が広い.
--      また, 型が項に依存しているといえるため, 依存型理論という. もう少し見よう.」
